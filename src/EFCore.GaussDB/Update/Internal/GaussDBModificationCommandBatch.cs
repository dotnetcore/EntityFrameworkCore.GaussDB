...
/// <summary>
///     The GaussDB-specific implementation for <see cref="ModificationCommandBatch" />.
/// </summary>
/// <remarks>
...
/// </remarks>
public class GaussDBModificationCommandBatch : ReaderModificationCommandBatch
{
    /// <summary>
    ///     Constructs an instance of the <see cref="GaussDBModificationCommandBatch" /> class.
    /// </summary>
    public GaussDBModificationCommandBatch(
    ...
    protected override void AddParameter(IColumnModification columnModification)
    {
        ...
        // sets. So we avoid adding GaussDBParameters for output parameters - we'll just retrieve and propagate the values below when
    ...
    {
        var GaussDBReader = (GaussDBDataReader)reader.DbDataReader;

#pragma warning disable 618
        Debug.Assert(
            GaussDBReader.Statements.Count == ModificationCommands.Count,
            $"Reader has {GaussDBReader.Statements.Count} statements, expected {ModificationCommands.Count}");
        ...
        try
        {
            ...
            while (commandIndex < ModificationCommands.Count)
            {
                ...
                if (ResultSetMappings[commandIndex].HasFlag(ResultSetMapping.HasResultRow))
                {
                    ...
                    onResultSet = async
                        ? await GaussDBReader.NextResultAsync(cancellationToken).ConfigureAwait(false)
                        : GaussDBReader.NextResult();
                }
                else
                {
...
#pragma warning disable 618
                    if (GaussDBReader.Statements[commandIndex].Rows != 1 && command.StoreStoredProcedure is null)
                    {
                        if (async)
                        {
                            await ThrowAggregateUpdateConcurrencyExceptionAsync(reader, commandIndex, 1, 0, cancellationToken)
