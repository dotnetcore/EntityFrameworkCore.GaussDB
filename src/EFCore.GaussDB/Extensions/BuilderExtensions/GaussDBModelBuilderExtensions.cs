...
/// <summary>
///     GaussDB-specific extension methods for <see cref="ModelBuilder" />.
/// </summary>
public static class GaussDBModelBuilderExtensions
{
    #region HiLo
        ...
        bool fromDataAnnotation = false)
    {
        ...
        return modelBuilder.CanSetAnnotation(GaussDBAnnotationNames.HiLoSequenceName, name, fromDataAnnotation)
            && modelBuilder.CanSetAnnotation(GaussDBAnnotationNames.HiLoSequenceSchema, schema, fromDataAnnotation);
    }
    ...
    public static ModelBuilder UseIdentityAlwaysColumns(this ModelBuilder modelBuilder)
    {
        ...
        model.SetValueGenerationStrategy(GaussDBValueGenerationStrategy.IdentityAlwaysColumn);
        model.SetSequenceNameSuffix(null);
        model.SetSequenceSchema(null);
    ...
    public static ModelBuilder UseIdentityByDefaultColumns(this ModelBuilder modelBuilder)
    {
        ...
        model.SetValueGenerationStrategy(GaussDBValueGenerationStrategy.IdentityByDefaultColumn);
        model.SetSequenceNameSuffix(null);
        model.SetSequenceSchema(null);
        model.SetHiLoSequenceName(null);
        model.SetHiLoSequenceSchema(null);
        ...
        bool fromDataAnnotation = false)
    {
        if (modelBuilder.CanSetValueGenerationStrategy(valueGenerationStrategy, fromDataAnnotation))
        {
            ...
            {
                if (modelBuilder.CanSetAnnotation(GaussDBAnnotationNames.SequenceNameSuffix, null)
                    && modelBuilder.CanSetAnnotation(GaussDBAnnotationNames.SequenceSchema, null))
                {
        ...
        string? schema = null)
    {
        ...
        nameSuffix ??= GaussDBModelExtensions.DefaultSequenceNameSuffix;

        model.SetValueGenerationStrategy(GaussDBValueGenerationStrategy.Sequence);
...
#pragma warning restore CS0618
        ...
        bool fromDataAnnotation = false)
    {
        ...
        return modelBuilder.CanSetAnnotation(
            GaussDBAnnotationNames.DefaultColumnCollation, collation, fromDataAnnotation);
    }
    ...
    // See: https://github.com/GaussDB/GaussDB/blob/dev/src/GaussDB/TypeMapping/TypeMapperBase.cs#L132-L138
    private static string GetTypePgName<TEnum>(IGaussDBNameTranslator nameTranslator)
        where TEnum : struct, Enum
        => typeof(TEnum).GetCustomAttribute<PgNameAttribute>()?.PgName ?? nameTranslator.TranslateTypeName(typeof(TEnum).Name);

    // See: https://github.com/GaussDB/GaussDB/blob/dev/src/GaussDB/TypeHandlers/EnumHandler.cs#L118-L129
    private static string[] GetMemberPgNames<TEnum>(IGaussDBNameTranslator nameTranslator)
        where TEnum : struct, Enum
        => typeof(TEnum)
