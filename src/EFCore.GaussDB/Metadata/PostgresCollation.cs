...
public class PostgresCollation
{
    ...
    private static string BuildAnnotationName(string? schema, string name)
        => schema is not null
            ? $"{GaussDBAnnotationNames.CollationDefinitionPrefix}{schema}.{name}"
            : $"{GaussDBAnnotationNames.CollationDefinitionPrefix}{name}";
    ...
    public static IEnumerable<PostgresCollation> GetCollations(IReadOnlyAnnotatable annotatable)
        => Check.NotNull(annotatable, nameof(annotatable))
            .GetAnnotations()
            .Where(a => a.Name.StartsWith(GaussDBAnnotationNames.CollationDefinitionPrefix, StringComparison.Ordinal))
        ...
        Deserialize(IAnnotation? annotation)
    {
        ...
        var schemaAndName = annotation.Name.Substring(GaussDBAnnotationNames.CollationDefinitionPrefix.Length).Split('.');
