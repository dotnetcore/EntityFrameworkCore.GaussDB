...
public class PostgresRange
{
    ...
    private static string BuildAnnotationName(string? schema, string name)
        => schema is not null
            ? $"{GaussDBAnnotationNames.RangePrefix}{schema}.{name}"
            : $"{GaussDBAnnotationNames.RangePrefix}{name}";
    ...
    public static IEnumerable<PostgresRange> GetPostgresRanges(IReadOnlyAnnotatable annotatable)
        => Check.NotNull(annotatable, nameof(annotatable))
            .GetAnnotations()
            .Where(a => a.Name.StartsWith(GaussDBAnnotationNames.RangePrefix, StringComparison.Ordinal))
        ...
        Deserialize(IAnnotation? annotation)
    {
        ...
        var schemaAndName = annotation.Name.Substring(GaussDBAnnotationNames.RangePrefix.Length).Split('.');
