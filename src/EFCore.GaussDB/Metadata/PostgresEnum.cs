...
public class PostgresEnum
{
    ...
    private static string BuildAnnotationName(string? schema, string name)
        => schema is not null
            ? $"{GaussDBAnnotationNames.EnumPrefix}{schema}.{name}"
            : $"{GaussDBAnnotationNames.EnumPrefix}{name}";
    ...
    public static IEnumerable<PostgresEnum> GetPostgresEnums(IReadOnlyAnnotatable annotatable)
        => Check.NotNull(annotatable, nameof(annotatable))
            .GetAnnotations()
            .Where(a => a.Name.StartsWith(GaussDBAnnotationNames.EnumPrefix, StringComparison.Ordinal))
    ...
    private static (string? Schema, string? Name, string[]? Labels) Deserialize(IAnnotation? annotation)
    {
        ...
        var schemaAndName = annotation.Name.Substring(GaussDBAnnotationNames.EnumPrefix.Length).Split('.');
