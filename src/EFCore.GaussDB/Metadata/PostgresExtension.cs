...
public class PostgresExtension
{
    ...
    internal static string BuildAnnotationName(string? schema, string name)
        => schema is not null
            ? $"{GaussDBAnnotationNames.PostgresExtensionPrefix}{schema}.{name}"
            : $"{GaussDBAnnotationNames.PostgresExtensionPrefix}{name}";
    ...
    public static IEnumerable<PostgresExtension> GetPostgresExtensions(IReadOnlyAnnotatable annotatable)
        => Check.NotNull(annotatable, nameof(annotatable))
            .GetAnnotations()
            .Where(a => a.Name.StartsWith(GaussDBAnnotationNames.PostgresExtensionPrefix, StringComparison.Ordinal))
    ...
    private static (string? Schema, string? Name, string? Version) Deserialize(IAnnotation? annotation)
    {
        ...
        var schemaAndName = annotation.Name.Substring(GaussDBAnnotationNames.PostgresExtensionPrefix.Length).Split('.');
