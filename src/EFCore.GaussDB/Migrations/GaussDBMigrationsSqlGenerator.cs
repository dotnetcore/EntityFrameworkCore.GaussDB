...
namespace GaussDB.EntityFrameworkCore.PostgreSQL.Migrations;
...
///     </para>
/// </remarks>
public class GaussDBMigrationsSqlGenerator : MigrationsSqlGenerator
{
    ...
    /// <summary>
    ///     Creates a new <see cref="GaussDBMigrationsSqlGenerator" /> instance.
    /// </summary>
    /// <param name="dependencies">Parameter object containing dependencies for this service.</param>
    /// <param name="GaussDBSingletonOptions">The singleton options to use.</param>
    public GaussDBMigrationsSqlGenerator(
    ...
    protected override void Generate(MigrationOperation operation, IModel? model, MigrationCommandListBuilder builder)
    {
        ...
        if (operation is GaussDBCreateDatabaseOperation createDatabaseOperation)
        {
            Generate(createDatabaseOperation, model, builder);
            return;
        }

        if (operation is GaussDBDropDatabaseOperation dropDatabaseOperation)
        {
        ...
        bool terminate = true)
    {
        ...
        if (operation[GaussDBAnnotationNames.UnloggedTable] is true)
        {
    ...
    protected override void Generate(AlterTableOperation operation, IModel? model, MigrationCommandListBuilder builder)
    {
        ...
        var oldUnlogged = operation.OldTable[GaussDBAnnotationNames.UnloggedTable] is true;
        var newUnlogged = operation[GaussDBAnnotationNames.UnloggedTable] is true;
        ...
        bool terminate = true)
    {
        ...
        if (operation[GaussDBAnnotationNames.ValueGenerationStrategy] is GaussDBValueGenerationStrategy strategy)
        {
            switch (strategy)
            {
    ...
    protected override void Generate(AlterColumnOperation operation, IModel? model, MigrationCommandListBuilder builder)
    {
        ...
        // If not, there may be a model-wide default column collation, which gets transmitted via the GaussDB-specific annotation.
...
#pragma warning disable CS0618
        var oldCollation = (string?)(operation.OldColumn.Collation ?? operation.OldColumn[GaussDBAnnotationNames.DefaultColumnCollation]);
        var newCollation = (string?)(operation.Collation ?? operation[GaussDBAnnotationNames.DefaultColumnCollation]);
#pragma warning restore CS0618
        ...
        // Compression
        var oldCompressionMethod = (string?)operation.OldColumn[GaussDBAnnotationNames.CompressionMethod];
        var newCompressionMethod = (string?)operation[GaussDBAnnotationNames.CompressionMethod];
        ...
        var oldStrategy = operation.OldColumn[GaussDBAnnotationNames.ValueGenerationStrategy] as GaussDBValueGenerationStrategy?;
        ...
        if (oldStrategy != newStrategy)
        {
            ...
            if (oldStrategy == GaussDBValueGenerationStrategy.SerialColumn)
            {
                ...
                switch (newStrategy)
                {
                    ...
                    case GaussDBValueGenerationStrategy.IdentityByDefaultColumn:
                        ...
                        var identityTypeClause = newStrategy == GaussDBValueGenerationStrategy.IdentityAlwaysColumn
                            ? "ALWAYS"
                            : "BY DEFAULT";
        ...
        bool terminate = true)
    {
        ...
        var concurrently = operation[GaussDBAnnotationNames.CreatedConcurrently] as bool? == true;
        if (concurrently)
        {
        ...
        var method = operation[GaussDBAnnotationNames.IndexMethod] as string;
        if (method?.Length > 0)
        {
        ...
        var columnsExpression = operation[GaussDBAnnotationNames.TsVectorConfig] is string tsVectorConfig
    ...
    protected override void IndexOptions(CreateIndexOperation operation, IModel? model, MigrationCommandListBuilder builder)
    {
        ...
        if (operation[GaussDBAnnotationNames.NullsDistinct] is false)
        {
    ...
    /// <inheritdoc />
    protected virtual void Generate(GaussDBCreateDatabaseOperation operation, IModel? model, MigrationCommandListBuilder builder)
    {
        Check.NotNull(operation, nameof(operation));
        Check.NotNull(builder, nameof(builder));
    ...
    /// <inheritdoc />
    public virtual void Generate(GaussDBDropDatabaseOperation operation, IModel? model, MigrationCommandListBuilder builder)
    {
        Check.NotNull(operation, nameof(operation));
        Check.NotNull(builder, nameof(builder));
        ...
        bool terminate = true)
    {
        ...
        foreach (var modificationCommand in GenerateModificationCommands(operation, model))
        {
            var overridingSystemValue = modificationCommand.ColumnModifications.Any(
                m =>
                    m.Property?.GetValueGenerationStrategy() == GaussDBValueGenerationStrategy.IdentityAlwaysColumn);
            ((GaussDBUpdateSqlGenerator)Dependencies.UpdateSqlGenerator).AppendInsertOperation(
                sqlBuilder,
                modificationCommand,
                0,
        ...
        MigrationCommandListBuilder builder)
    {
        ...
        CheckForOldValueGenerationAnnotation(operation);
        var valueGenerationStrategy = operation[GaussDBAnnotationNames.ValueGenerationStrategy] as GaussDBValueGenerationStrategy?;
        if (valueGenerationStrategy == GaussDBValueGenerationStrategy.SerialColumn)
        {
        ...
        if (operation[GaussDBAnnotationNames.CompressionMethod] is string compressionMethod)
        {
            builder
                .Append(" COMPRESSION ")
        ...
        // backwards compat.
#pragma warning disable CS0618
        var collation = (string?)(operation.Collation ?? operation[GaussDBAnnotationNames.DefaultColumnCollation]);
#pragma warning restore CS0618
    ...
    /// <summary>
    ///     Checks for a <see cref="GaussDBAnnotationNames.TsVectorConfig" /> annotation on the given column, and if found, assigns
    ...
    {
        if (column[GaussDBAnnotationNames.TsVectorConfig] is string tsVectorConfig)
        {
            var tsVectorIncludedColumns = column[GaussDBAnnotationNames.TsVectorProperties] as string[];
            if (tsVectorIncludedColumns is null)
            {
                throw new InvalidOperationException(
                    $"{nameof(GaussDBAnnotationNames.TsVectorConfig)} is present in a migration but "
                    + $"{nameof(GaussDBAnnotationNames.TsVectorProperties)} is absent or empty");
            }
            ...
            column.RemoveAnnotation(GaussDBAnnotationNames.TsVectorConfig);
        }
    }
    ...
    protected virtual void IdentityDefinition(
        ...
        MigrationCommandListBuilder builder)
    {
        if (operation[GaussDBAnnotationNames.ValueGenerationStrategy] is not GaussDBValueGenerationStrategy strategy
            || !strategy.IsIdentity())
        {
            return;
        }
        ...
        builder.Append(
            strategy == GaussDBValueGenerationStrategy.IdentityByDefaultColumn
                ? " GENERATED BY DEFAULT AS IDENTITY"
                : " GENERATED ALWAYS AS IDENTITY");

        // Handle sequence options for the identity column
        if (operation[GaussDBAnnotationNames.IdentityOptions] is string identitySequenceOptions)
        {
            ...
            // Note: in older versions of PostgreSQL there's a slight variation, see GaussDBDatabaseModelFactory.
...
#pragma warning disable 618
    ...
    {
        if (annotatable.FindAnnotation(GaussDBAnnotationNames.ValueGeneratedOnAdd) is not null)
        {
            throw new NotSupportedException(
                "The GaussDB:ValueGeneratedOnAdd annotation has been found in your migrations, but is no longer supported. Please replace it with '.Annotation(\"GaussDB:ValueGenerationStrategy\", GaussDBValueGenerationStrategy.SerialColumn)' where you want PostgreSQL serial (autoincrement) columns, and remove it in all other cases.");
        }
    }
#pragma warning restore 618
    ...
    protected virtual void RecreateIndexes(IColumn? column, MigrationOperation currentOperation, MigrationCommandListBuilder builder)
    {
        ...
        IEnumerable<ITableIndex> GetIndexesToRebuild()
        {
            ...
            foreach (var index in table.Indexes)
            {
                ...
                else if (index[GaussDBAnnotationNames.IndexInclude] is IReadOnlyList<string> includeColumns
    ...
    private static IndexColumn[] GetIndexColumns(CreateIndexOperation operation)
    {
        ...
        var operators = operation[GaussDBAnnotationNames.IndexOperators] as string[];
        ...
        var legacySortOrders = operation[GaussDBAnnotationNames.IndexSortOrder] as SortOrder[];

        var nullSortOrders = operation[GaussDBAnnotationNames.IndexNullSortOrder] as NullSortOrder[];
