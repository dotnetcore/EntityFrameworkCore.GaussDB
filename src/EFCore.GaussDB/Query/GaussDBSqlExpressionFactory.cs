...
namespace GaussDB.EntityFrameworkCore.PostgreSQL.Query;

/// <inheritdoc />
public class GaussDBSqlExpressionFactory : SqlExpressionFactory
{
    ...
    /// <summary>
    ///     Creates a new instance of the <see cref="GaussDBSqlExpressionFactory" /> class.
    /// </summary>
    /// <param name="dependencies">Parameter object containing dependencies for this class.</param>
    public GaussDBSqlExpressionFactory(SqlExpressionFactoryDependencies dependencies)
        : base(dependencies)
    {
        _typeMappingSource = (GaussDBTypeMappingSource)dependencies.TypeMappingSource;
    ...
    public override SqlExpression? ApplyTypeMapping(SqlExpression? sqlExpression, RelationalTypeMapping? typeMapping)
    {
        ...
        if (!GaussDBTypeMappingSource.LegacyTimestampBehavior
            && (typeMapping is GaussDBTimestampTypeMapping && sqlExpression?.TypeMapping is GaussDBTimestampTzTypeMapping
                || typeMapping is GaussDBTimestampTzTypeMapping && sqlExpression?.TypeMapping is GaussDBTimestampTypeMapping))
        {
        ...
        RelationalTypeMapping? typeMapping)
    {
        ...
        return new PgArrayIndexExpression(
            ...
            pgArrayIndexExpression.Array.TypeMapping is GaussDBArrayTypeMapping arrayMapping
        ...
        RelationalTypeMapping? typeMapping)
    {
        ...
        switch (operatorType)
        {
            ...
            case PgExpressionType.RangeIsAdjacentTo:
            {
                ...
                if (left.Type == right.Type
                    || left.TypeMapping is GaussDBArrayTypeMapping
                    || right.TypeMapping is GaussDBArrayTypeMapping)
                {
        ...
        (SqlExpression, SqlExpression) InferContainmentMappings(SqlExpression container, SqlExpression containee)
        {
            ...
            if (containeeMapping is null)
            {
                ...
                {
                    GaussDBRangeTypeMapping rangeTypeMapping => rangeTypeMapping.SubtypeMapping,
                    GaussDBMultirangeTypeMapping multirangeTypeMapping
                        => containee.Type.IsGenericType && containee.Type.GetGenericTypeDefinition() == typeof(GaussDBRange<>)
    ...
    private SqlExpression ApplyTypeMappingOnNewArray(
        ...
        RelationalTypeMapping? typeMapping)
    {
        var arrayTypeMapping = typeMapping as GaussDBArrayTypeMapping;
        if (arrayTypeMapping is null && typeMapping is not null)
        {
            throw new ArgumentException($"Type mapping {typeMapping.GetType().Name} isn't an {nameof(GaussDBArrayTypeMapping)}");
        }
        ...
        foreach (var expression in pgNewArrayExpression.Expressions)
        {
            ...
            else if (expressionTypeMapping.StoreType != elementTypeMapping.StoreType)
            {
                ...
                {
                    throw new InvalidOperationException(
                        GaussDBStrings.HeterogeneousTypesInNewArray(
        ...
        else
        {
            ...
            arrayTypeMapping = (GaussDBArrayTypeMapping?)_typeMappingSource.FindMapping(
