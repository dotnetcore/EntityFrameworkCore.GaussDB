...
/// </summary>
public class GaussDBByteArrayMethodTranslator : IMethodCallTranslator
{
    ...
    ///     doing so can result in application failures when updating to a new Entity Framework Core release.
    /// </summary>
    public GaussDBByteArrayMethodTranslator(ISqlExpressionFactory sqlExpressionFactory)
    {
        ...
        IDiagnosticsLogger<DbLoggerCategory.Query> logger)
    {
        ...
        if (method.IsGenericMethod && arguments[0].TypeMapping is GaussDBByteArrayTypeMapping typeMapping)
        {
