...
/// </summary>
public class GaussDBDateTimeMethodTranslator : IMethodCallTranslator
{
    ...
    private static readonly MethodInfo DateTime_Distance
        = typeof(GaussDBDbFunctionsExtensions).GetRuntimeMethod(
            nameof(GaussDBDbFunctionsExtensions.Distance), new[] { typeof(DbFunctions), typeof(DateTime), typeof(DateTime) })!;
    ...
    private static readonly MethodInfo DateOnly_Distance
        = typeof(GaussDBDbFunctionsExtensions).GetRuntimeMethod(
            nameof(GaussDBDbFunctionsExtensions.Distance), new[] { typeof(DbFunctions), typeof(DateOnly), typeof(DateOnly) })!;
    ...
    ///     doing so can result in application failures when updating to a new Entity Framework Core release.
    /// </summary>
    public GaussDBDateTimeMethodTranslator(
        ...
        IReadOnlyList<SqlExpression> arguments)
    {
        if (instance is null)
        {
            if (method == DateTime_SpecifyKind)
            {
                ...
                if (typeMapping is not GaussDBTimestampTypeMapping and not GaussDBTimestampTzTypeMapping)
                {
                ...
                {
                    return typeMapping is GaussDBTimestampTypeMapping
                ...
                {
                    return typeMapping is GaussDBTimestampTzTypeMapping
                        ? _sqlExpressionFactory.AtUtc(arguments[0])
                        : arguments[0];
                }
            }
        ...
        IReadOnlyList<SqlExpression> arguments)
    {
        if (instance is null)
        {
            if (method == DateOnly_FromDateTime)
            {
                ...
                {
                    GaussDBTimestampTypeMapping => arguments[0],
                    GaussDBTimestampTzTypeMapping => _sqlExpressionFactory.AtUtc(arguments[0]),
