...
/// </summary>
public class GaussDBJsonDomTranslator : IMemberTranslator, IMethodCallTranslator
{
    ...
    ///     doing so can result in application failures when updating to a new Entity Framework Core release.
    /// </summary>
    public GaussDBJsonDomTranslator(
        ...
        IDiagnosticsLogger<DbLoggerCategory.Query> logger)
    {
        ...
        if (member == RootElement && instance is ColumnExpression { TypeMapping: GaussDBJsonTypeMapping } column)
        {
    ...
    public virtual SqlExpression? Translate(
        ...
        IDiagnosticsLogger<DbLoggerCategory.Query> logger)
    {
        if (method.DeclaringType != typeof(JsonElement) || instance?.TypeMapping is not GaussDBJsonTypeMapping mapping)
        {
            return null;
        }
