...
/// </remarks>
public class GaussDBDateTimeMemberTranslator : IMemberTranslator
{
    ...
    /// </summary>
    public GaussDBDateTimeMemberTranslator(IRelationalTypeMappingSource typeMappingSource, GaussDBSqlExpressionFactory sqlExpressionFactory)
    {
        ...
        IDiagnosticsLogger<DbLoggerCategory.Query> logger)
    {
        ...
        if (member.Name == nameof(DateTime.Date))
        {
            ...
            switch (instance)
            {
                case { TypeMapping: GaussDBTimestampTypeMapping }:
                case { TypeMapping: GaussDBTimestampTzTypeMapping } when GaussDBTypeMappingSource.LegacyTimestampBehavior:
                ...
                case { TypeMapping: GaussDBTimestampTzTypeMapping }:
                    return _sqlExpressionFactory.Function(
                        "date_trunc",
                ...
                case { TypeMapping: GaussDBDateTimeDateTypeMapping }:
        ...
        {
            // Legacy behavior
            nameof(DateTime.Now) when GaussDBTypeMappingSource.LegacyTimestampBehavior
                => UtcNow(),
            nameof(DateTime.UtcNow) when GaussDBTypeMappingSource.LegacyTimestampBehavior
    ...
    private bool TryConvertAwayFromTimestampTz(SqlExpression timestamp, [NotNullWhen(true)] out SqlExpression? result)
    {
        switch (timestamp)
        {
            ...
            case { TypeMapping: GaussDBTimestampTypeMapping or GaussDBDateTimeDateTypeMapping or GaussDBTimeTypeMapping }:
