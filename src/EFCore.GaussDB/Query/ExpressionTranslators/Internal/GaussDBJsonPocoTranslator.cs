...
/// </summary>
public class GaussDBJsonPocoTranslator : IMemberTranslator, IMethodCallTranslator
{
    ...
    ///     doing so can result in application failures when updating to a new Entity Framework Core release.
    /// </summary>
    public GaussDBJsonPocoTranslator(
    ...
    public virtual SqlExpression? Translate(
        ...
        IDiagnosticsLogger<DbLoggerCategory.Query> logger)
    {
        if (instance?.TypeMapping is not GaussDBJsonTypeMapping && instance is not PgJsonTraversalExpression)
        {
            return null;
        }
    ...
    public virtual SqlExpression? TranslateMemberAccess(SqlExpression instance, SqlExpression member, Type returnType)
    {
        return instance switch
        {
            ...
            ColumnExpression { TypeMapping: GaussDBJsonTypeMapping } columnExpression
                => ConvertFromText(
        ...
        SqlExpression ConvertFromText(SqlExpression expression, Type returnType)
            => _typeMappingSource.FindMapping(returnType.UnwrapNullableType(), _model) switch
            {
                ...
                GaussDBArrayTypeMapping => expression,
    ...
    public virtual SqlExpression? TranslateArrayLength(SqlExpression expression)
    {
        switch (expression)
        {
            case ColumnExpression { TypeMapping: GaussDBJsonTypeMapping mapping }:
            ...
            case PgJsonTraversalExpression traversal:
                ...
                var jsonMapping = (GaussDBJsonTypeMapping)traversal.Expression.TypeMapping!;
                return _sqlExpressionFactory.Function(
