...
internal static class EnumerableMethods
{
    ...
    static EnumerableMethods()
    {
        ...
        FirstWithoutPredicate = GetMethod(nameof(Enumerable.First), 1, types => [typeof(IEnumerable<>).MakeGenericType(types[0])]);
        ...
        //         nameof(Enumerable.Average), 0, _ => new[] { typeof(IEnumerable<>).MakeGenericType(type) });
        ...
        //         nameof(Enumerable.Sum), 0, _ => new[] { typeof(IEnumerable<>).MakeGenericType(type) });
